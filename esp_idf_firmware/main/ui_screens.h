...
// ---- Screens ----
inline lv_obj_t *scr_login, *scr_home, *scr_scan, *scr_tasks, *scr_inventory, *scr_conn, *scr_settings;
...
inline void ui_update_tasks_from_json(const char *json_str) {
  ...
  if (cJSON_IsArray(root)) {
    ...
    for (int i = 0; i < size && i < MAX_TASKS; i++) {
      ...
      if (strcmp(t.prio, "High") == 0) {
        t.prio_color = COLOR_DANGER;
      } else if (strcmp(t.prio, "Medium") == 0) {
        t.prio_color = COLOR_SAFFRON;
      } else {
...
inline void ui_update_users_from_json(const char *json_str) {
  ...
  if (cJSON_IsArray(root)) {
    ...
    for (int i = 0; i < size && i < MAX_USERS; i++) {
      ...
      if (cJSON_IsString(id) && id->valuestring) strncpy(u.id, id->valuestring, sizeof(u.id) - 1);
      if (cJSON_IsString(name) && name->valuestring) strncpy(u.name, name->valuestring, sizeof(u.name) - 1);
      if (cJSON_IsString(role) && role->valuestring) strncpy(u.role, role->valuestring, sizeof(u.role) - 1);
      if (cJSON_IsString(pin) && pin->valuestring) strncpy(u.pin, pin->valuestring, sizeof(pin) - 1);
...
static void _ptt_action_cb(lv_event_t *e) {
  ...
  if (ptt_active_state) {
    ...
    lv_label_set_text(lbl, LV_SYMBOL_AUDIO "  TALKING");
  } else {
    ...
    lv_label_set_text(lbl, LV_SYMBOL_AUDIO "  TAP TO TALK");
  }
}
...
static void update_tasks_ui() {
  ...
  for (int i = 0; i < current_task_count; i++) {
    ...
    if (strcmp(current_tasks[i].status, "complete") == 0) {
      lv_obj_set_style_text_decor(name, LV_TEXT_DECOR_STRIKETHROUGH, 0);
...
static void build_login_screen() {
  ...
  lv_obj_add_event_cb(btn_login, [](lv_event_t *e) {
    ...
    if ((strcmp(user, "123") == 0 || strcmp(user, "1") == 0) && (strcmp(pass, "123") == 0 || strcmp(pass, "1234") == 0)) {
      found = true; auth_ok = true;
      matched_name = (strcmp(user, "123") == 0) ? "Operator 123" : "Admin 1";
    } else {
      for (int i = 0; i < global_user_count; i++) {
        if (strcmp(global_users[i].id, user) == 0) {
          found = true;
          if (strcmp(global_users[i].pin, pass) == 0) { auth_ok = true; matched_name = global_users[i].name; }
          break;
        }
      }
    }
    if (auth_ok) {
      ...
      if (!found) { if (label_login_err) lv_label_set_text(label_login_err, "! User ID not found"); }
      else { if (label_login_err) lv_label_set_text(label_login_err, "! Incorrect password"); }
    }
  }, LV_EVENT_CLICKED, NULL);
}
...
inline void uiShowScanResult(const std::string &sku) {
  ...
  if (label_last_scan_flag) {
    ...
    std::string txt = "Progress: " + std::to_string(scan_counter) + " / " + std::to_string(scan_target);
  ...
  if (label_scan_count_home) {
    std::string txt = "Scans this session: " + std::to_string(scan_counter);
    lv_label_set_text(label_scan_count_home, txt.c_str());
  }
  _load_scr_direct(scr_scan, "Scan");
}
...
inline void uiLoginViaScan(const std::string &raw) {
  ...
  for (int i = 0; i < global_user_count; i++) {
    if (std::string(global_users[i].id) == username) {
      found = true;
  ...
  if (!found || !auth_ok) {
    ...
    ESP_LOGI("UI", "[login] QR auth failed for user='%s'\n", username.c_str());
    return;
  }
  ...
  ESP_LOGI("UI", "[login] QR login OK — user='%s'\n", logged_in_user);
